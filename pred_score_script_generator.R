pred_score_bash_file = '~/pred_score4.bash'

pred_command = 'python predict.py'
score_commnad = 'python score.py '
data_dir = '/home/chen/data/deepGRN/raw/'
models_path = '/home/chen/data/deepGRN/results/reproduce3/'
info_file = '/home/chen/data/deepGRN/deepgrn_score.csv'
slurm_prefix = ''
...
for(i in list.files(models_path,pattern = '.+\\.csv',recursive = T,full.names = T)){
    ...
    for(pred_cell_name in cell_names){
        ...
        pred_command1 <- paste0(slurm_prefix,'\n',pred_command,' -i ',data_dir,' -p ',pred_region_file,
        ...
        write(paste0(pred_command1,'\n',score_commnad1), file =paste0(out_dir,'/predict_score_local.sh'),append = F)
...
for(i in list.files(models_path,pattern = '.+\\.csv',recursive = T,full.names = T)){
    out_dir <- gsub('(.+/).+','\\1',i)
    his <- read.csv(i)
    if(((nrow(his)-which.max(his$val_acc))>=patience)){
        # training finished
        print(i)
      
        if(!file.exists(paste0(out_dir,'/','already_predict'))){
            file.create(paste0(out_dir,'/','already_predict'))
            print(paste0('bash ',out_dir,'/predict_score_local.sh'))
            ps <- paste0(ps,'\nbash ',out_dir,'/predict_score_local.sh')
        }
...
write(ps, file = pred_score_bash_file,append = F)
