...
import get_model, res_model, utils
...
#    parser.add_argument('--single_strand', '-ss', action='store_true',help='single_strand')
    ...
    parser.add_argument('--val_negative_ratio', '-vn', type=int, required=False,help='val_negative_ratio',default=4)
...
def main():
    ...
    if module_outfile.is_file():
        ...
        epochs_trained = h.shape[0]
        wait_iter = h.shape[0] - h['val_auPRC'].idxmax()-1
        model = load_model(output_dir+'/best_model.h5',custom_objects={'Attention1D': get_model.Attention1D})
        print('model relay from epoch '+str(epochs_trained))
    else:
        wait_iter = 0
        best_auprc = 0
        epochs_trained = 0
        print('model trained from new model')
        n_channel=6
        if not unique35:
            n_channel = 5
...
# python train_relay.py -i /home/chen/data/deepGRN/raw/ -t CTCF -ap resnetv1_lstm -o /home/chen/data/deepGRN/results/res_ctcf/ --plot_model --use_cudnn --unique35 -k 20 -nr 32 -dl 0 --use_peak
