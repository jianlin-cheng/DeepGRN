...
def scikitlearn_calc_auROC(y_true, y_score):
    positives = np.array(y_true == 1)
    return roc_auc_score(positives, y_score)

def auc_roc(y_true, y_pred):
    return tf.py_func(scikitlearn_calc_auROC, (y_true, y_pred), tf.double)
...
def auc_prc(y_true, y_pred):
    return tf.py_func(scikitlearn_calc_auPRC, (y_true, y_pred), tf.double)
