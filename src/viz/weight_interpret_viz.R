...
for(data_path in list.dirs(data_path_all,recursive = F)){
  if(!file.exists(paste0(data_path,'fc.tiff'))){
    ...
    prob_fw_var_slt <- prob_fw_var>=quantile(prob_fw_var,0.9)
    prob_rc_var <- apply(prob_rc, 2, var)
    prob_rc_var_slt <- prob_rc_var>=quantile(prob_rc_var,0.9)
    ...
    g_dnase <- ggplot() + geom_point(aes(x = 1:nrow(xy_fw), y = val_dnase, colour = val_dnase), size = 3) +xlab('')+ylab('')+ theme(legend.position="none")
    # +stat_smooth(aes(x = 1:nrow(xy_fw), y = val_dnase), method = "lm",formula = y ~ poly(x, 23), se = FALSE)
    
    g <- ggplot() + geom_point(aes(x = 1:nrow(xy_fw), y = fold_change, colour = fold_change), size = 3) +xlab('')+ylab('')+ theme(legend.position="none")
